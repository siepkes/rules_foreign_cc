...
Based upon the 'freebsd_commands.bzl' file.

Bash is one of the prerequisites for Bazel:
...
def copy_dir_contents_to_dir(source, target):
    # Beause illumos `cp` doesn't have `--no-copy-directory`, we have to
...
def replace_symlink(file):
    return """\
if [[ -L "{file}" ]]; then
  # illumos does not have a native 'readlink' command. So this is likely GNU
  # Core utils readlink. See: https://www.illumos.org/issues/16399
